...
/**
 * Typed query helper. Single entry point for every SQL call in the app.
 * Uses sql.query() which accepts (string, params[]) unlike the tagged
 * template form. Returns { rows } to match the pg.QueryResult shape that
 * all query modules already depend on.
 */
